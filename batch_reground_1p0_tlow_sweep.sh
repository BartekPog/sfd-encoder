#!/bin/bash
# =============================================================================
# batch_reground_1p0_tlow_sweep.sh
#
# Follow-up to batch_reground_1p0_hidden_newag_array.sh: t_fix=0.1 with cfg=1.4
# (no-hidden B-FT AG) gave FID 1.21 on 1p0_v4_..._repg_1p5 @ 80K, with FID
# trending downward as t_fix decreased. This sweep pushes t_fix to {0.0, 0.05}
# at cfg ∈ {1.3, 1.4, 1.5} to see if the trend keeps going (auxiliary-loss
# interpretation) or saturates.
#
# Model: 1p0_v4_mse0001_noisy_enc_nocurr_shift1_repg_1p5 @ 80000
# AG:    configs/sfd/autoguidance_b/inference_ft.yaml (no-hidden B FT @ 80K)
# Schedule: encode-reground + sphere clamp + --reground_fixed_enc_noise
# Sampler:  Euler 100 steps, FID50K balanced
#
# Total: 2 t_fix × 3 cfg = 6 array tasks (max parallel 16)
# =============================================================================
...
T_FIX_VALUES=(0.0 0.05)
CFG_SCALES=(1.3 1.4 1.5)

# ---- Build matrix: T_FIX|CFG_SCALE ----
MATRIX=()
for T_FIX in "${T_FIX_VALUES[@]}"; do
    for CFG_SCALE in "${CFG_SCALES[@]}"; do
        MATRIX+=("${T_FIX}|${CFG_SCALE}")
    done
done
NUM_TASKS=${#MATRIX[@]}
...
echo "============================================="
echo "  Reground t_fix-low sweep — no-hidden B FT autoguidance"
...
for ENTRY in "${MATRIX[@]}"; do
    IFS='|' read -r T_FIX CFG_SCALE <<< "${ENTRY}"
    echo "  task: t_fix=${T_FIX} cfg=${CFG_SCALE}"
done
...
echo "Reground t-low task \${SLURM_ARRAY_TASK_ID}: t_fix=\${T_FIX} cfg=\${CFG_SCALE}"
