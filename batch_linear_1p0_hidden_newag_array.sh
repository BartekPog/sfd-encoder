#!/bin/bash
# =============================================================================
# batch_linear_1p0_hidden_newag_array.sh
#
# SLURM job-array sweep: linear hidden schedule on 2 1p0B hidden models with
# the new B-FT autoguidance. No HRG. CFG sweep over {1.1, 1.3}.
#
# Models:
...
#
# Schedule: linear + sphere clamp, no --hidden_rep_guidance
# Sampler:  Euler 100 steps, FID50K balanced
#
# Note: --cfg_noise_hidden is a no-op here (it's only wired into the
# encode-reground sample fn, and even there is bypassed when autoguidance
# is enabled). It is therefore not part of this matrix; the reground array
# will sweep it instead.
#
# Total: 2 models × 2 cfg = 4 array tasks (max parallel 4)
# =============================================================================
...
# ---- Build matrix: MODEL_ENTRY|CFG_SCALE ----
MATRIX=()
for MODEL_ENTRY in "${MODELS[@]}"; do
    for CFG_SCALE in "${CFG_SCALES[@]}"; do
        MATRIX+=("${MODEL_ENTRY}|${CFG_SCALE}")
    done
done
NUM_TASKS=${#MATRIX[@]}
...
echo "============================================="
echo "  Linear hidden sweep — new B-FT autoguidance"
...
for ENTRY in "${MATRIX[@]}"; do
    IFS='|' read -r CONFIG_PATH TRAIN_EXP_NAME CKPT_STEP CFG_SCALE <<< "${ENTRY}"
    ...
    echo "  task: ${TRAIN_EXP_NAME} @ ${CKPT_STEP} cfg=${CFG_SCALE}"
done

mkdir -p jobs job_outputs

JOBNAME="lin_1p0_newag"
...
echo "Linear (new AG) task \${SLURM_ARRAY_TASK_ID}: \${TRAIN_EXP_NAME} @ \${CKPT_STEP} cfg=\${CFG_SCALE}"
